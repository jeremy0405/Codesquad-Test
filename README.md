...
**Revision 번호**: 73 // hash 값 : 계속수정중..

저장하기 불러오기 기능은 끝까지 구현 하지 못했다. 지도 데이터 변환 프로그램과 되돌리기 기능 및 되돌리기 취소 기능은 구현했다.

저장하기 불러오기 기능 중 입력에 따른 명령 분리 부분은 구현했다. (1S, 2S 를 입력 받고 입력에 따라 분류하는 것까지)

사용자의 입력 명령에 따라 저장하기, 불러오기 기능을 구현하지 못했다.
...
지도 데이터를 바이너리 코드로 암호화한 후 map_enc.txt에 저장했다.

소코반 게임을 실행하면 MapReader 클래스에서 Decode 클래스를 통해 map_enc.txt를 복호화하여 지도 데이터를 읽어서 게임을 실행한다.

#### EncryptMain

메인메서드를 가지고 있는 메인클래스이다. Encrypt 클래스에 map.txt 파일을 넘겨준 후 암호화되어져서 나온 String을 새로운 map_enc.txt에 저장한다.

#### Encrypt

"map.txt"를 입력받아 바이너리 코드로 암호화 한 후 String으로 반환한다.

#### Decode

소코반 게임의 MapReader클래스에서 초기에 mapdata를 읽어올 때 Decode를 이용해서 암호화되어 있는 map_enc.txt 파일을 복호화하는 역할을 한다.

복호화 한 데이터를 String으로 반환한다.

#### MapReader

`readMapFile()` 메서드에서 Decode 클래스를 통해 복호화된 String을 mapdata에 저장할 수 있도록 전처리한다. 
